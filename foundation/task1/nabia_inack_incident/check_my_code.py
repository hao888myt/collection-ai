...
critical_cases = [
    (
        "低于阈值一点",
        lambda: battle.check_critical_hit(battle.CRITICAL_HIT_THRESHOLD - 1) is False,
    ),
    (
        "正好达到阈值",
...
invalid_damage = [
    (
        "负数基础伤害",
        lambda: raises_value_error(lambda: battle.calculate_final_damage(-1, 0)),
    ),
    (
        "负数防御值",
        lambda: raises_value_error(lambda: battle.calculate_final_damage(1, -1)),
    ),
]
